<template>
  <div class="hello">
    ...
    <h3><span @click="setUserName('allen2001')">{{ userName }}</span> test mock data</h3>
    <ul>
      <li v-for="item in dataList" :key="item.url"><a :href="item.url" rel="noopener">{{ item.name }}</a></li>
    </ul>
    <h3>vueuse play</h3>
    <div>
      <h4>Mouse: {{ x }} x {{ y }}</h4>
    ...
    <div>
      <TestForm v-model:name="formData.name" v-model:age="formData.age" v-model:sex="formData.sex"></TestForm>
      <p>{{ formData }}</p>
...
export default {
  ...
  setup(props, context) {
    ...
    const getMockData = async () => {
      ...
      context.emit('update:msg', 'get mock data succ')
    }
